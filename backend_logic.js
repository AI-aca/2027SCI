...
예시: "독립된 활동 주제(제목) 누락. 무의미한 문자 포함으로 주제 적합성 보완 필요."`;
...
async function generateAIQuestions(studentId, type) {
  try {
    ...
    const formatConstraint = "\n\n[출력 서식 강제 규칙]\n모든 예상 질문 세트는 반드시 아래의 마크다운(Markdown) 서식을 엄격하게 지켜 출력하십시오. 질문 어투는 실제 면접관이 질문하듯 '~해 주세요.', '~하세요.' 형식의 정중하고 명확한 구어체/경어체를 사용하십시오.\n\n### 📝 Q[번호]. [[분야명]] (이곳에 질문의 핵심 요약 키워드 작성)\n- **🗣️ 면접 질문**: 실제 면접관이 질문하듯 학생에게 던지는 메인 면접 질문 내용 ('~해 주세요.', '~하세요.' 등의 구어체)\n- **🎯 출제 의도**: 이 질문을 통해 확인하고자 하는 학생의 역량 및 평가 요소 서술\n- **🔗 꼬리 질문**: 메인 질문에 이어질 수 있는 1~2개의 추가 압박 질문";
...
(🚨특히, JSON에 없는 기술·가정, 정보 등의 과목을 스스로 유추하여 감점으로 지적하는 행위를 절대 금지합니다.)
...
async function savePersonalStatement(payload) {
  try {
    ...
    for (const qNum in newSnapshots) {
      ...
      const { error: insErr } = await window.supabaseClient.from('personal_statements').upsert(row, { onConflict: 'student_link,question_no' });
