...
async function updatePrompts() {
  ...
  const updatePromises = data.map(async (row) => {
    ...
    // Replace logic (handling different newline characters just in case)
    let newText = originalText.replace(
      /반드시 아래 JSON 포맷을 유지하여 순수 JSON만 반환하십시오\.\r?\n\{/,
      '반드시 아래 JSON 포맷을 유지하여 순수 JSON만 반환하십시오. (🚨단, 정답 배열에 값을 넣기 전에 "_think_scratchpad" 항목에 제공된 생기부 문서 전체를 훑으며 발견한 모든 영역별 증거 문장들을 먼저 자유롭게 메모하며 빠진 것이 없는지 뇌를 워밍업할 것)\n{\n  "_think_scratchpad": "여기에 문서 전체를 스캔하며 발견한 관련 증거 문장들을 먼저 자유롭게 메모할 것",'
    );

    // 2. Patch the updated prompt back to Supabase
    ...
    if (patchRes.ok) {
      console.log(`[성공] ${row.setting_key} 업데이트 완료`);
    } else {
