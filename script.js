...
// 🌐 전역 합불 통신 함수 신설
window.updatePassStatus = async function(studentPhone, passType, passValue) {
  try {
    const result = await ApiClient.post('updatePassStatus', { studentId: studentPhone, passType, passValue }, { hideLoader: true });
    if (!result.success) {
...
window.triggerPdfUpload = function(studentPhone) {
  PDF_TARGET_STUDENT = studentPhone;
        ...
        if (CURRENT_ROLE === '학생') {
           ...
           td.innerHTML = `
             <div style="display: flex; align-items: center; justify-content: center; gap: 6px;">
               ...
               <select class="form-control" style="width:auto; padding:2px 4px; font-size:14px; background:var(--bg-card); color:var(--text-main);" ${disabled} onchange="window.updatePassStatus('${student.studentPhone}', '${col.key}', this.value); const icon = document.getElementById('icon-${student.studentLink}-${col.key}'); if (this.value === '합') { icon.className = 'fa-solid fa-circle-check'; icon.style.color = '#10b981'; } else if (this.value === '불') { icon.className = 'fa-solid fa-circle-xmark'; icon.style.color = '#ef4444'; } else { icon.className = 'fa-solid fa-circle-dot'; icon.style.color = '#94a3b8'; }">
 ...
 */
function openEditStudent(studentPhone) {
  const student = STUDENTS_LIST.find(s => s.studentPhone === studentPhone || s.studentLink === studentPhone);
  if (!student) return;
  ...
  if (deleteGroup) {
    ...
    document.getElementById('btn-archive-student').onclick = function() {
      if(confirm('이 학생을 명단에서 숨기시겠습니까? (파일과 데이터는 구글 시트에 보존됩니다.)')) {
        ApiClient.post('archiveStudent', { studentPhone: student.studentPhone }).then(res => {
          if(res.success) {
            alert('성공적으로 숨김 처리되었습니다.');
            document.getElementById('modal-register').style.display = 'none';
            STUDENTS_LIST = STUDENTS_LIST.filter(s => s.studentPhone !== student.studentPhone);
            renderMainTable();
          } else {
            alert('오류: ' + res.error);
          }
    ...
    btnHardDelete.onclick = function() {
      if(confirm('정말 삭제하시겠습니까? 구글 드라이브의 자소서 파일, 생기부 PDF, 생기부 AI 산출근거, 메인 행까지 모두 완벽하게 영구 삭제됩니다.\n\n이 작업은 절대 되돌릴 수 없습니다!')) {
        ApiClient.post('hardDeleteStudent', { studentPhone: student.studentPhone }).then(res => {
          if(res.success) {
...
/**
 * 23가지 생기부 점수 상세 아코디언 토글 렌더러
 */
function toggleScoreAccordion(studentPhone, totalScore) {
  if (CURRENT_ROLE === '학생' || CURRENT_ROLE === '게스트') {
    alert('상세 채점 내역 조회 권한이 없습니다. 교사나 관리자만 조회 가능합니다.');
    return;
  }
  openScoreDetailsModal(studentPhone);
}
...
async function openPersonalStatementModal(studentPhone, initialTab = 'manual') {
  ACTIVE_PS_STUDENT = studentPhone;
  ...
  if (CURRENT_ROLE === '교사' || CURRENT_ROLE === '관리자') {
    ...
    saveBtn.style.display = 'inline-block'; // 문항별 피드백 저장 허용
    saveBtn.textContent = '문항별 피드백 저장하기';
      ...
      if (!unlockBtn) {
        ...
        unlockBtn.onclick = async () => {
          if (confirm('최종 제출을 반려하고 수정을 허용하시겠습니까?')) {
            ...
            try {
              const res = await ApiClient.post('unlockPersonalStatement', { studentId: studentPhone, adminPassword: inputPw });
              if (res && res.success) {
...
async function openInterviewPractice(studentPhone, mode) {
  ACTIVE_INTERVIEW_STUDENT = studentPhone;
  ...
  try {
    questionsData = await ApiClient.post('getAIQuestions', { studentId: studentPhone });
  } catch (e) {
...
function bindEventHandlers() {
  ...
  document.getElementById('btn-submit-register').onclick = async () => {
    ...
    if (!studentData.center || !studentData.name || !studentData.school || !studentData.targetSchool) {
      alert('센터명, 학생명, 현재 학교, 지원 예정 과학고는 필수 기재 사항입니다.');
      return;
    }
    ...
    try {
      if (isEditMode) {
        // 기존 학생 수정 로직 (mock/연동)
        await ApiClient.post('updateStudent', { studentData, originalPhone: document.getElementById('reg-student-phone').value });
  ...
  if (genAIQuestionsPsBtn) {
    genAIQuestionsPsBtn.onclick = async () => {
      ...
      const student = STUDENTS_LIST.find(s => String(s.studentPhone) === String(ACTIVE_INTERVIEW_STUDENT) || String(s.studentLink) === String(ACTIVE_INTERVIEW_STUDENT));
  ...
  document.getElementById('btn-submit-ps-final').onclick = async () => {
    ...
    // 💡 작성 안 된 문항 검증 로직 시작
    const student = STUDENTS_LIST.find(s => s.studentPhone === ACTIVE_PS_STUDENT || s.studentLink === ACTIVE_PS_STUDENT);
    const schoolMap = window.SCHOOL_QUESTIONS_MAP || [];
...
/**
 * 23가지 평가 항목 점수 산정근거 모달 열기
 */
async function openScoreDetailsModal(studentPhone) {
  if (CURRENT_ROLE === '학생' || CURRENT_ROLE === '게스트') {
    alert('상세 채점 내역 조회 권한이 없습니다. 교사나 관리자만 조회 가능합니다.');
    return;
  }
  ...
  const student = STUDENTS_LIST.find(s => String(s.studentPhone) === String(studentPhone) || String(s.studentLink) === String(studentPhone));
  if (!student) return;
  ...
  try {
    const reqPw = sessionStorage.getItem('user_pw') || '';
    const res = await ApiClient.post('getScoreDetailsBasis', { studentId: studentPhone, clientRole: CURRENT_ROLE, authPw: reqPw });
    if (res.success) {
...
async function runBulkAIEval() {
  ...
  for (let i = 0; i < targetStudents.length; i++) {
    ...
    try {
      await ApiClient.post('evaluateStudentRecord', { studentId: student.studentPhone, recordText: null });
...
async function runBulkAIFeedback() {
  ...
  for (let i = 0; i < targetStudents.length; i++) {
    ...
    try {
      await ApiClient.post('generateAIFeedback', { studentId: student.studentPhone });
...
async function runBulkAIQuestions(mode) {
  ...
  for (let i = 0; i < targetStudents.length; i++) {
    ...
    try {
      await ApiClient.post('generateAIQuestions', { studentId: student.studentPhone, type: isPsMode ? '자소서' : '생기부' });
      await new Promise(r => setTimeout(r, 3500));
    } catch (e) {
